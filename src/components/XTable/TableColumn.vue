...
<template>
  ...
  <el-table-column v-else v-bind="col">
    <template #default="{ row, $index }">
      <!---图片 (START)-->
      <!-- 如需更改图片size，可自行配置参数 -->
      <el-image v-if="col.type === 'image'" preview-teleported :hide-on-click-modal="true"
        :preview-src-list="[row[col.prop!]]" :src="row[col.prop!]" fit="cover" class="w-9 h-9 rounded-lg" />
      <!---图片 (END)-->
      ...
      <template v-else-if="col.buttons?.length">
        <el-button v-for="(btn, index) in col.buttons" :key="index"  :type="btn.type"
          @click="handleAction(btn.command, { row, $index })">{{ btn.name }}</el-button>
      </template>
      <!-- 如果传递按钮数组，就展示按钮组 END-->
      <!-- render函数 (START) -->
    ...
    <template #header="{ column, $index }">
      ...
      <slot v-else-if="col.headerSlot" name="customHeader" :slotName="col.headerSlot" :column="column" :index="$index">
      </slot>
